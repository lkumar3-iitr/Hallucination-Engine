...
def packed_iou_scores(packed_masks, target, mask_areas=None):
    ...
    if mask_areas is None:
        ...
        unions = mask_areas + np.count_nonzero(target) - intersections
    return intersections / np.maximum(unions, 1)
...
class Selector:
    ...
    def predicted_mask(self, actor, camera, width, height, fov):
        ...
        # The static voxel mesh repeats vertices across adjacent faces.
        cached = getattr(self, '_projection_vertices', None)
        if cached is None or cached[0] is not self.faces:
            vertices, inverse = np.unique(self.faces.reshape(-1, 3), axis=0, return_inverse=True)
            cached = (self.faces, vertices, inverse)
            self._projection_vertices = cached
        uv, depth = project(cached[1], relative_matrix(actor, camera),
        ...
        polygons = np.rint(normalized).astype(np.int32)[cached[2]].reshape(-1, 4, 2)
        mask = np.zeros((self.size, self.size), np.uint8)
        # Axis-collapsed faces paint inclusive horizontal/vertical pixel runs.
        low = np.minimum(np.minimum(polygons[:, 0], polygons[:, 1]),
                         np.minimum(polygons[:, 2], polygons[:, 3]))
        high = np.maximum(np.maximum(polygons[:, 0], polygons[:, 1]),
                          np.maximum(polygons[:, 2], polygons[:, 3]))
        collapsed = np.any(low == high, axis=1)
        lo_run, hi_run = low[collapsed], high[collapsed]+1
        stride = self.size+1
        positive = np.concatenate((lo_run[:, 1]*stride+lo_run[:, 0],
                                   hi_run[:, 1]*stride+hi_run[:, 0]))
        negative = np.concatenate((lo_run[:, 1]*stride+hi_run[:, 0],
                                   hi_run[:, 1]*stride+lo_run[:, 0]))
        coverage = (np.bincount(positive, minlength=stride*stride)
                    - np.bincount(negative, minlength=stride*stride)).reshape(stride, stride)
        mask[:] = coverage.cumsum(axis=0).cumsum(axis=1)[:-1, :-1] > 0
        # A polygon adds nothing if its entire inclusive bounding rectangle is filled.
        integral = np.pad(mask.astype(np.int64).cumsum(0).cumsum(1), ((1, 0), (1, 0)))
        lo_remaining, hi_remaining = low[~collapsed], high[~collapsed]+1
        lx, ly = lo_remaining.T
        hx, hy = hi_remaining.T
        covered = integral[hy, hx]-integral[ly, hx]-integral[hy, lx]+integral[ly, lx]
        needed = covered != (hx-lx)*(hy-ly)
        remaining = np.ascontiguousarray(polygons[~collapsed][needed])
        records = remaining.reshape(-1, 8).view(np.dtype((np.void, 32))).ravel()
        _, unique_indices = np.unique(records, return_index=True)
        for polygon in remaining[unique_indices]:
    ...
    def select(self, actor, camera, width, height, fov):
        ...
        areas = getattr(self, '_mask_area_cache', None)
        if areas is None or areas[0] is not self.packed_masks:
            areas = (self.packed_masks, POPCOUNT_U8[self.packed_masks].sum(axis=1))
            self._mask_area_cache = areas
        scores = packed_iou_scores(self.packed_masks[eligible_indices], predicted,
                                   areas[1][eligible_indices])
