...
COMMON = Path(__file__).resolve().parents[2] / "driving_models/common"
if str(COMMON) not in sys.path:
    sys.path.insert(0, str(COMMON))
import he_camera_renderer as geometry


class UnsupportedCameraGeometry(ValueError):
    """A pose outside the native sprite renderer's supported domain."""

    def __init__(self, reason, distance_m):
        self.reason = reason
        self.distance_m = float(distance_m)
        super().__init__(f'{reason}: target distance {self.distance_m:.8g} m')
...
def aimed_camera(actor_tf, camera_tf, center, extents=None):
    ...
    if extents is not None:
        ...
        if np.linalg.norm(target_local - camera_local) < 1e-5:
            raise UnsupportedCameraGeometry("camera_inside_or_on_physical_bbox", 0.)
    target_world = actor_matrix @ np.r_[target_local, 1.0]
    ray = target_world[:3]-camera_world[:3]
    distance = np.linalg.norm(ray)
    if distance < .05:
        reason = ("camera_near_physical_bbox_surface" if extents is not None
                  else "camera_near_aiming_center")
        raise UnsupportedCameraGeometry(reason, distance)
