"""Recorder-compatible compositor for the production HE sprite renderer."""
...
class HESpriteRendererCompositor:
    def __init__(self, distance_selection_mode="linear", bottom_y_offset_px=0,
        ...
        if bottom_y_offset_px != 0 or silhouette_scale != 1 or viewpoint_lateral_sign != 1:
            raise ValueError("The HE sprite renderer uses calibrated geometry; manual overrides are unsupported")
        if warp_scale_mode != "independent":
            raise ValueError("The HE sprite renderer requires independent box fitting")
        self.engines = {}
    ...
    def render(self, base_rgb, camera_tf, active_actors, width, height, fov, scene_depth_m=None):
        if scene_depth_m is not None:
            raise ValueError("The HE sprite renderer does not yet implement scene-depth occlusion")
        ...
        for order, state in enumerate(states):
            ...
            if close is not None:
                ...
                metadata.update(
                    renderer_backend="he_sprite_renderer_v1",
                ...
                metadata.update(
                    renderer_backend="he_sprite_renderer_v1", reason="" if rendered else "outside_viewport",
                    ...
                    anchor_mode="he_centered_camera_reprojection", projection_mode="pure_camera_rotation",
                    box={"visible": rendered, "depth_m": state.distance_forward_m,
                         "camera_right_m": state.rel_x_m, "camera_up_m": state.rel_y_m},
