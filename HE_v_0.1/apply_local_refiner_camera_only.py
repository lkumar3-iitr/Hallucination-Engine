...
def find_camera_only_meta_by_frame(camera_only_sequence, frame_id):
    for item in camera_only_sequence:
...
def create_preview_row(clean_bgr, real_bgr, rough_bgr, refined_bgr):
    panels = [
        ("clean", clean_bgr),
        ("real", real_bgr),
        ("camera_only", rough_bgr),
        ("camera_only_refined", refined_bgr),
    ]
...
def load_refiner_model(checkpoint_path):
    print("[ApplyCamOnlyRefiner] Loading checkpoint:", checkpoint_path)
    ...
    print("[ApplyCamOnlyRefiner] Loaded model.")
    print("[ApplyCamOnlyRefiner] Base channels:", base_channels)
    print("[ApplyCamOnlyRefiner] Device:", DEVICE)
...
def apply_refiner_to_camera_only(pair_run_dir, checkpoint_path):
    ...
    camera_only_sequence = sequence_metadata.get("he_camera_only_sequence", [])

    clean_dir = os.path.join(pair_run_dir, "clean")
    real_dir = os.path.join(pair_run_dir, "real_object")
    rough_dir = os.path.join(pair_run_dir, "he_camera_only")

    refined_dir = os.path.join(pair_run_dir, "he_camera_only_refined")
    preview_dir = os.path.join(pair_run_dir, "he_camera_only_refined_preview")
    ...
    print("[ApplyCamOnlyRefiner] Pair run:", pair_run_dir)
    print("[ApplyCamOnlyRefiner] Frames:", len(clean_paths))
    print("[ApplyCamOnlyRefiner] Output:", refined_dir)
    ...
    with torch.no_grad():
        for idx, clean_path in enumerate(clean_paths):
            ...
            cam_meta = find_camera_only_meta_by_frame(
                camera_only_sequence,
                frame_id,
            )
            ...
            if not is_valid_box(box):
                ...
                refined_metadata.append({
                    "frame": frame_id,
                    "status": "no_valid_camera_only_box",
                    "box_2d": box,
                })
                continue
            ...
            if idx % 20 == 0:
                print(f"[ApplyCamOnlyRefiner] Processed frame {idx}/{len(clean_paths)}")
    ...
    refined_meta_path = os.path.join(
        refined_meta_dir,
        "he_camera_only_refined_metadata.json",
    )

    with open(refined_meta_path, "w", encoding="utf-8") as f:
        json.dump(refined_meta, f, indent=4)

    print("[ApplyCamOnlyRefiner] Done.")
    print("[ApplyCamOnlyRefiner] Refined frames:", refined_dir)
    print("[ApplyCamOnlyRefiner] Metadata:", refined_meta_path)
