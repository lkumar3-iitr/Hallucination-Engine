...
DEFAULT_MAX_RESIDUAL_DELTA = 0.25
...
def find_latest_residual_refiner_checkpoint(base_dir=CHECKPOINT_BASE_DIR):
    """
    Prefer latest local_refiner_residual checkpoint.
    """

    if not os.path.isdir(base_dir):
        raise RuntimeError(f"Refiner checkpoint base not found: {base_dir}")

    ckpts = glob.glob(
        os.path.join(
            base_dir,
            "local_refiner_residual_*",
            "checkpoints",
            "*.pth"
        )
    )
    ...
    if not ckpts:
        raise RuntimeError(
            f"No residual refiner checkpoint files found in: {base_dir}\n"
            f"Expected path like: refiner_checkpoints/local_refiner_residual_xxx/checkpoints/*.pth"
        )
...
class ResidualUNetRefiner(nn.Module):
    """
    Same architecture as train_local_refiner_residual.py.

    Input:
        clean RGB + rough RGB + mask = 7 channels

    Output:
        residual RGB correction in [-max_delta, +max_delta]
    """

    def __init__(self, in_ch=7, out_ch=3, base_ch=32, max_delta=DEFAULT_MAX_RESIDUAL_DELTA):
        super().__init__()
...
def square_expanded_crop_box(box, image_w, image_h, expand_factor):
    """
    Build square crop around predicted box.
    """
...
def make_mask_from_rough_difference(clean_bgr, rough_bgr, box, crop_box):
    """
    Create mask from difference between predicted HE image and clean image.

    This follows the inserted sprite region.
    """
...
def paste_refined_crop_back(
    ...
    # Soft mask, but not too strong. We want sharper object preservation.
...
def create_preview_row(clean_bgr, real_bgr, rough_bgr, refined_bgr):
    """
    Side-by-side row:
        clean | real | box_pred | residual_refined
    """
...
def load_residual_refiner_model(checkpoint_path):
    print("[ApplyResidualRefiner] Loading checkpoint:", checkpoint_path)

    checkpoint = torch.load(checkpoint_path, map_location=DEVICE)

    train_meta = checkpoint.get("train_meta", {})
    base_channels = int(train_meta.get("base_channels", 32))
    max_delta = float(train_meta.get("max_residual_delta", DEFAULT_MAX_RESIDUAL_DELTA))

    model_version = train_meta.get("model_version", "")

    if "residual" not in model_version:
        print(
            "[ApplyResidualRefiner] Warning: checkpoint model_version does not contain 'residual'. "
            "Make sure this is a residual refiner checkpoint."
        )
    ...
    print("[ApplyResidualRefiner] Loaded residual refiner.")
    print("[ApplyResidualRefiner] Base channels:", base_channels)
    print("[ApplyResidualRefiner] Max delta:", max_delta)
    print("[ApplyResidualRefiner] Device:", DEVICE)
...
def apply_residual_refiner_to_box_predicted(pair_run_dir, checkpoint_path):
    model, train_meta = load_residual_refiner_model(checkpoint_path)
    ...
    box_pred_metadata_path = os.path.join(
        pair_run_dir,
        "metadata",
    ...
    if not os.path.exists(box_pred_metadata_path):
        raise RuntimeError(
            f"Missing predicted box metadata: {box_pred_metadata_path}\n"
    ...
    print("[ApplyResidualRefiner] Pair run:", pair_run_dir)
    print("[ApplyResidualRefiner] Frames:", len(clean_paths))
    print("[ApplyResidualRefiner] Output:", refined_dir)
    ...
    with torch.no_grad():
        for idx, clean_path in enumerate(clean_paths):
            ...
            pred_meta = find_box_pred_meta_by_frame(
                predicted_frames,
                frame_id,
            )
            ...
            if idx % 20 == 0:
                print(f"[ApplyResidualRefiner] Processed frame {idx}/{len(clean_paths)}")

    refined_meta = {
        "model_version": "residual_local_refiner_v1",
        "pair_run_dir": pair_run_dir,
        "residual_refiner_checkpoint_path": checkpoint_path,
        ...
        "note": "Residual refiner applies small correction to he_box_predicted using predicted box.",
    ...
    print("[ApplyResidualRefiner] Done.")
    print("[ApplyResidualRefiner] Refined frames:", refined_dir)
    print("[ApplyResidualRefiner] Preview:", preview_dir)
    print("[ApplyResidualRefiner] Metadata:", refined_meta_path)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to residual local refiner checkpoint. If omitted, latest residual refiner checkpoint is used.",
    )
    ...
    if args.checkpoint is None:
        checkpoint_path = find_latest_residual_refiner_checkpoint(CHECKPOINT_BASE_DIR)
    else:
