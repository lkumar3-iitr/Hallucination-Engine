...
def build_native_frame_difference_mask(
    ...
    """
    CARLA-native target silhouette.

    The target is the ONLY dynamic actor changed between these two
    synchronized camera poses. A pixel belongs to the target iff CARLA's
    instance-segmentation RGB annotation changes at that pixel.

    This uses all annotation channels (semantic R + instance G/B), so it
    still works when a blueprint reports unexpected semantic tags or has
    zero/unhelpful G/B instance bytes.
    """
