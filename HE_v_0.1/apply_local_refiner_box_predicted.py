...
def find_latest_refiner_checkpoint(base_dir=CHECKPOINT_BASE_DIR):
    if not os.path.isdir(base_dir):
    ...
    if not ckpts:
        raise RuntimeError(f"No local refiner checkpoint files found in: {base_dir}")
...
def create_preview_row(clean_bgr, real_bgr, rough_bgr, refined_bgr):
    """
    Side-by-side row:
        clean | real | box_predicted | box_predicted_refined
    """
...
def load_refiner_model(checkpoint_path):
    print("[ApplyBoxPredRefiner] Loading local refiner checkpoint:", checkpoint_path)
    ...
    print("[ApplyBoxPredRefiner] Loaded local refiner.")
    print("[ApplyBoxPredRefiner] Base channels:", base_channels)
    print("[ApplyBoxPredRefiner] Device:", DEVICE)
...
def apply_refiner_to_box_predicted(pair_run_dir, checkpoint_path):
    ...
    refined_dir = os.path.join(pair_run_dir, "he_box_predicted_refined")
    preview_dir = os.path.join(pair_run_dir, "he_box_predicted_refined_preview")
    ...
    print("[ApplyBoxPredRefiner] Pair run:", pair_run_dir)
    print("[ApplyBoxPredRefiner] Frames:", len(clean_paths))
    print("[ApplyBoxPredRefiner] Output:", refined_dir)
    ...
    with torch.no_grad():
        for idx, clean_path in enumerate(clean_paths):
            ...
            if idx % 20 == 0:
                print(f"[ApplyBoxPredRefiner] Processed frame {idx}/{len(clean_paths)}")

    refined_meta = {
        "pair_run_dir": pair_run_dir,
        "local_refiner_checkpoint_path": checkpoint_path,
        ...
        "note": "real_box_2d_for_eval_only is not used for refinement; predicted box is used.",
    ...
    refined_meta_path = os.path.join(
        pair_run_dir,
        "metadata",
        "he_box_predicted_refined_metadata.json",
    )

    save_json(refined_meta_path, refined_meta)

    print("[ApplyBoxPredRefiner] Done.")
    print("[ApplyBoxPredRefiner] Refined frames:", refined_dir)
    print("[ApplyBoxPredRefiner] Preview:", preview_dir)
    print("[ApplyBoxPredRefiner] Metadata:", refined_meta_path)
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to local refiner checkpoint. If omitted, latest local refiner checkpoint is used.",
    )
    ...
    if args.checkpoint is None:
        checkpoint_path = find_latest_refiner_checkpoint(CHECKPOINT_BASE_DIR)
    else:
        checkpoint_path = args.checkpoint

    apply_refiner_to_box_predicted(
        pair_run_dir=pair_run_dir,
