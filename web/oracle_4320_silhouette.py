...
def pair_metrics(candidate: np.ndarray, target: np.ndarray) -> dict:
    ...
    c_count = max(1, int(np.count_nonzero(c_edge)))
    t_count = max(1, int(np.count_nonzero(t_edge)))

    c_to_t = float(np.sum(t_dt[c_edge > 0]) / c_count)
    t_to_c = float(np.sum(c_dt[t_edge > 0]) / t_count)
...
def preload_normalized_bank(
    ...
    # float16 keeps the cache reasonably small.
    edge_dt = np.zeros((n, s, s), dtype=np.float16)
    ...
    for i, candidate in enumerate(candidates):
        ...
        masks[i] = norm
        edges[i] = edge
        edge_dt[i] = dt.astype(np.float16)
