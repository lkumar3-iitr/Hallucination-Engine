...
class AstraCompositor:
    ...
    def render(self, base_rgb, camera_tf, active_actors, width, height, fov, scene_depth_m=None):
        ...
        for order, state in enumerate(states):
            ...
            image, alpha, metadata = engine.render(cv2.cvtColor(rgb, cv2.COLOR_RGB2BGR), actor_tf, camera_tf, fov)
            ...
            metadata.update(
                renderer_backend="astra_passby_v1", reason="" if len(xs) else "outside_viewport",
                ...
                box={"visible": bool(len(xs)), "depth_m": state.distance_forward_m,
                     "camera_right_m": state.rel_x_m, "camera_up_m": state.rel_y_m},
                cartesian_close={"active": False, "reason": "astra_4320_backend"})
            ...
            actor_results.append(ActorRenderResult(**shared, render_order=order,
                                                   rendered=bool(len(xs)), he_metadata=metadata))
