...
class Selector:
    ...
    def build_hull(self, spacing=0.035, angle_step=10, tolerance_px=1):
        config = {"version": 1, "bank": self.fingerprint, "spacing": spacing,
        ...
        for index, row in enumerate(self.rows):
            if float(row["distance_m"]) != 5 or int(float(row["angle_deg"])) % angle_step:
                continue
    ...
    def select(self, actor, camera, width, height, fov):
        ...
        scores = iou_scores(self.masks, predicted)
        # Silhouettes alone confuse opposite sides; enforce physically compatible appearance.
        eligible = angle_delta <= 35
        ranked = scores - 0.002*(angle_delta/35)**2
        ranked[~eligible] = -np.inf
        ...
        return {"index": selected, "sprite_path": str(self.path(self.rows[selected])),
                ...
                "predicted_iou": float(scores[selected]), "baseline_index": baseline,
