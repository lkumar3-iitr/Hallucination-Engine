...
def aimed_camera(actor_tf, camera_tf, center):
    center_world = np.asarray(actor_tf.get_matrix()) @ np.r_[center, 1.0]
    ray = center_world[:3]-np.array([camera_tf.location.x, camera_tf.location.y, camera_tf.location.z])
...
class PassbyRenderer:
    ...
    def render(self, background_bgr, actor_tf, camera_tf, fov, box_mode="hull", box_provider=None):
        ...
        virtual, distance = aimed_camera(actor_tf, camera_tf, self.selector.center)
