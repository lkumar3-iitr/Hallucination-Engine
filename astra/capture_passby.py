...
def main():
    ...
    try:
        ...
        for yaw in (0, -60, -90):
            ...
            setup = {"schema": "astra_complete_passby_v1", "fps": 20, "frame_count": 125,
            ...
            with (directory / "frames.jsonl").open("w") as log:
                for index in range(125):
                    ...
                    vehicle = annotation[:, :, 2] == 14
                    if instance_key is None:
                        keys, counts = np.unique(ids[vehicle], return_counts=True)
                    ...
                    mask = (vehicle & (ids == instance_key)).astype(np.uint8)*255
                    ...
                    if np.any((hidden_annotation[:, :, 2] == 14) & (hidden_ids == instance_key)):
