...
Accepted renderer candidate: `astra_passby_v1`. See
...
Lighting, reflections, RGB fidelity, temporal stability, external occlusion,
and near-plane intersection remain separate issues. The current selector
explicitly rejects a hull crossing the camera near plane. The external-box
compositor handles ordinary viewport clipping, but clipped-frame selector
accuracy is not part of the reported validation. Existing-bank source capture
calibration is required. Current reconstruction uses an axis-aligned physical
bbox and the available 5 m slice; this implementation is validated on Tesla,
not yet other assets. Runtime is about 0.12 seconds per tested pose on this
machine, excluding startup, so real-time optimization remains necessary.
