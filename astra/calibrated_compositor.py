...
class CalibratedCompositor:
    bank_path = Path(__file__).resolve().parent/"artifacts/bus_aimed_close_v3"
    diagnostic_log = None
    bank_paths = {}
    ...
    def render(self, base_rgb, camera_tf, active_actors, width, height, fov, scene_depth_m=None):
        ...
        for order, (actor, state) in enumerate(pairs):
            ...
            if candidate_bank is None:
                if str(bank) not in self.base.engines:
                    ...
                    self.base.engines[str(bank)] = HESpriteRenderer(
                        bank, root/"artifacts/production_baseline_cache",
            ...
            meta.update(renderer_backend="astra_calibrated_hull_candidate_v1", rendered=rendered,
